...
class Fuel_auth extends Fuel_base_library {
	 ...
	 * @return	void
	 */	
	public function set_valid_user_fb($valid_user)
	{
		$this->CI->load->library('session');

		// set minimal session data
		$session_data = array();
		$session_data['id'] = $valid_user['cli_id'];
		$session_data['user_name'] = $valid_user['cli_title'];
		$session_data['fbid'] = $valid_user['cli_fbid'];
		$this->CI->session->set_userdata($this->get_session_namespace(), $session_data);
	}
